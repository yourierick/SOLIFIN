...
class BoostService
{
    ...
    public function boostPublication($publication, $days, $amount, $publicationType)
    {
        ...
        try {
            ...
            $metadata = $this->prepareBoostMetadata($publication, $publicationType, $days, $amount, $paymentMethod, $paymentType);
            
            // Débiter le wallet utilisateur
            $this->debitUserWallet($wallet, $amount, $metadata);
            
            // Ajouter le montant au wallet system
            $this->updateWalletSystem($user, $amount, $metadata);
     ...
     * @return array
     */
    private function prepareBoostMetadata($publication, $publicationType, $days, $amount, $paymentMethod, $paymentType)
    {
        $publicationTypeLabel = $this->getPublicationTypeLabel($publicationType);
        
        return [
            'Opération' => "Boost de publication",
            'Id de la publication' => $publication->id,
     ...
     * @return void
     */
    private function debitUserWallet($wallet, $amount, $metadata)
    {
        // Utiliser la méthode withdrawFunds du modèle Wallet
        $wallet->withdrawFunds($amount, 'purchase', 'completed', $metadata);
    }
     ...
     * @return void
     */
    private function updateWalletSystem($user, $amount, $metadata)
    {
