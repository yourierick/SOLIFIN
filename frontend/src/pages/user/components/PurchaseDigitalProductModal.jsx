...
const PurchaseDigitalProductModal = ({
  ...
  const fetchFeePercentage = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get("/api/digital-products/purchase-fee-percentage");
    ...
    try {
      ...
      if (response.data) {
        ...
        if (response.data.message === 'Vous avez déjà acheté ce produit') {
          ...
          // Achat réussi
          Notifications.success(response.data.message || "Produit numérique acheté avec succès");
          ...
          <>
            ...
            >
              ...
              <Typography
                ...
                sx={{ fontWeight: 600 }}
              >
                {userWallet?.balance || "0.00"} {product.devise}
              </Typography>
            </Box>
