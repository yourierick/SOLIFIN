  ...
  const handleVerification = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (isExpired(response.data.ticket)) {
          toast.warning("Ce ticket est expiré!", {
            position: "top-right",
            autoClose: 5000,
          });
        }
        ...
        toast.error(response.data.message || "Ticket non trouvé ou invalide", {
          position: "top-right",
          autoClose: 5000,
        });
      }
    } catch (error) {
      const errorMessage = error.response?.data?.message || "Erreur lors de la vérification du ticket";
  ...
  const handleConsume = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setTicket({...ticket, consomme: true, date_consommation: new Date().toISOString()});
        toast.success("Ticket validé avec succès ! Le cadeau peut être remis.", {
          position: "top-right",
          autoClose: 5000,
        });
      } else {
        const errorMessage = response.data.message || "Erreur lors de la consommation du ticket";
        setError(errorMessage);
        toast.error(errorMessage, {
          position: "top-right",
          autoClose: 5000,
      ...
      const errorMessage = error.response?.data?.message || "Erreur lors de la consommation du ticket";
      setError(errorMessage);
      toast.error(errorMessage, {
        position: "top-right",
        autoClose: 5000,
  ...
  const fetchHistory = async (page = 1, filters = {}) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Notification pour les filtres appliqués
        ...
        const errorMessage = response.data.message || "Erreur lors du chargement de l'historique";
