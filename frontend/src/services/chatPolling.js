...
class ChatPollingService {
  ...
  adjustPollingInterval(roomId) {
    if (this.pollingIntervals[roomId]) {
      ...
      const interval = this.isWindowActive ? POLLING_INTERVALS.ACTIVE : POLLING_INTERVALS.INACTIVE;
  ...
  startMessagePolling(roomId, customInterval = null) {
    ...
    const interval = customInterval || (this.isWindowActive ? POLLING_INTERVALS.ACTIVE : POLLING_INTERVALS.INACTIVE);
  ...
  async fetchNewMessages(roomId) {
    try {
      ...
      if (messages && messages.length > 0) {
        ...
        this.lastMessageTimestamps[roomId] = new Date(latestMessage.created_at).getTime();
  ...
  async updateUserStatus() {
    try {
      await axios.post('/api/user/status/update');
    } catch (error) {
  ...
  async fetchUserStatuses() {
    try {
      ...
      console.error('Erreur lors de la récupération des statuts utilisateur:', error);
    }
  }
